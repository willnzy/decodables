...
3. User profile timezone (from database - async)
4. Default UTC
...
Usage:
    ...
    timezone = await get_request_timezone(request, user_id=user.get("id"))
"""
...
async def get_timezone_from_user_profile(user_id: Optional[str]) -> Optional[str]:
    """
    Get timezone from user's database profile via async DB lookup.

    Uses UserRepository.get_timezone() which queries the profiles table.

    Args:
        user_id: User ID

    Returns:
        str or None: Timezone if user has one set (not UTC)
    """
    if not user_id:
        return None
    ...
    try:
        ...
        tz = await user_repo.get_timezone(user_id)

        if tz and tz != "UTC":
            logger.debug(f"Timezone from user profile: {tz}")
            return tz
    except Exception as e:
        logger.warning(f"Failed to get user timezone from profile: {e}")
...
async def get_request_timezone(
    request: Request,
    ...
    4. User profile timezone (from database - async)
    5. Default (UTC)
    ...
    Example:
        ...
        async def purchase(request: Request, body: PurchaseRequest, user = Depends(get_current_user)):
            timezone = await get_request_timezone(
                request,
                user_id=user.get("id"),
    ...
    # 4. User profile (async DB lookup)
    profile_tz = await get_timezone_from_user_profile(user_id)
    if profile_tz:
        return profile_tz
...
async def get_timezone_info(request: Request, user_id: Optional[str] = None) -> dict:
    """
    ...
    """
    return {
        "resolved": await get_request_timezone(request, user_id),
        "sources": {
            "header": get_timezone_from_header(request),
            "cloudflare": get_timezone_from_cloudflare(request),
            "user_profile": await get_timezone_from_user_profile(user_id) if user_id else None,
        },
        "cloudflare_headers": {
...
async def get_request_timezone_with_country_fallback(
    request: Request,
    user_id: Optional[str] = None,
    timezone_override: Optional[str] = None,
    default: str = "UTC"
    ...
    tz = await get_request_timezone(request, user_id, timezone_override, default=None)
    if tz:
        return tz
