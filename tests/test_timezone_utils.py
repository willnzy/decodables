...
class TestGetRequestTimezone:
    """Tests for get_request_timezone function"""
    ...
    def test_header_precedence_over_cloudflare(self):
        ...
        result = get_request_timezone(request)
        
        assert result == "Europe/Paris"
    
    def test_cloudflare_fallback(self):
    ...
    @patch('timezone_utils.get_timezone_from_user_profile')
    def test_user_profile_fallback(self, mock_profile_tz):
        """User profile timezone should be used as fallback"""
        mock_profile_tz.return_value = "Australia/Sydney"
        request = self._create_mock_request({})
        
        result = get_request_timezone(request, user_id="user123")
        
        assert result == "Australia/Sydney"
        mock_profile_tz.assert_called_once_with("user123")
    ...
    def test_custom_default(self):
        ...
        result = get_request_timezone(request, default="Asia/Tokyo")
        
        assert result == "Asia/Tokyo"
    
    def test_invalid_override_uses_next_source(self):
...
class TestGetTimezoneInfo:
    """Tests for get_timezone_info function"""
    ...
    def test_returns_complete_info(self):
        ...
        info = get_timezone_info(request)
...
class TestGetRequestTimezoneWithCountryFallback:
    """Tests for get_request_timezone_with_country_fallback function"""
    ...
    def test_country_fallback_used(self):
        ...
        # Patch to prevent header/profile lookups
        with patch('timezone_utils.get_timezone_from_header', return_value=None), \
             patch('timezone_utils.get_timezone_from_cloudflare', return_value=None), \
             patch('timezone_utils.get_timezone_from_user_profile', return_value=None):
            
            result = get_request_timezone_with_country_fallback(request)
        
        assert result == "Asia/Tokyo"
    
    def test_timezone_header_preferred_over_country(self):
    ...
    def test_default_when_no_country(self):
        ...
        with patch('timezone_utils.get_timezone_from_header', return_value=None), \
            ...
            result = get_request_timezone_with_country_fallback(
                request, 
