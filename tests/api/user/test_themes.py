...
class TestThemesAPI:
    ...
    def test_get_themes_success(self, auth_headers):
        """获取 Themes 成功"""
        # TODO: 根据实际端点调整
        response = client.get("/api/v2/user/themes", headers=auth_headers)
    ...
    def test_get_themes_unauthorized(self):
        """未认证应返回 401"""
        response = client.get("/api/v2/user/themes")

        # 应该需要认证
        assert response.status_code in [401, 404]

    @patch('infrastructure.repositories.supabase')
    def test_themes_with_mock(self, mock_supabase, auth_headers):
        """使用 mock 测试 Themes"""
        # Mock Supabase 响应
