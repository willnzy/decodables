...
class TestTasksAPI:
    ...
    def test_get_tasks_success(self, auth_headers):
        """获取 Tasks 成功"""
        # TODO: 根据实际端点调整
        response = client.get("/api/v2/user/tasks", headers=auth_headers)
    ...
    def test_get_tasks_unauthorized(self):
        """未认证应返回 401"""
        response = client.get("/api/v2/user/tasks")

        # 应该需要认证
        assert response.status_code in [401, 404]

    @patch('services.db_service.supabase')
    def test_tasks_with_mock(self, mock_supabase, auth_headers):
        """使用 mock 测试 Tasks"""
        # Mock Supabase 响应
