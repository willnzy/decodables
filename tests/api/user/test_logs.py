...
class TestLogsAPI:
    ...
    def test_get_logs_success(self, auth_headers):
        """获取 Logs 成功"""
        # TODO: 根据实际端点调整
        response = client.get("/api/v2/user/logs", headers=auth_headers)
    ...
    def test_get_logs_unauthorized(self):
        """未认证应返回 401"""
        response = client.get("/api/v2/user/logs")

        # 应该需要认证
        assert response.status_code in [401, 404]

    @patch('infrastructure.repositories.supabase')
    def test_logs_with_mock(self, mock_supabase, auth_headers):
        """使用 mock 测试 Logs"""
        # Mock Supabase 响应
