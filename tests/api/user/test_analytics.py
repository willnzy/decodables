"""
测试 api/analytics_api.py

端点: POST /analytics/event
...
class TestAnalyticsAPI:
    ...
    def test_get_analytics_success(self, auth_headers):
        """获取 Analytics 成功"""
        # TODO: 根据实际端点调整
        response = client.get("/api/v2/user/analytics", headers=auth_headers)
    ...
    def test_get_analytics_unauthorized(self):
        """未认证应返回 401"""
        response = client.get("/api/v2/user/analytics")

        # 应该需要认证
        assert response.status_code in [401, 404]

    @patch('services.db_service.supabase')
    def test_analytics_with_mock(self, mock_supabase, auth_headers):
        """使用 mock 测试 Analytics"""
        # Mock Supabase 响应
