...
class MaintenanceScheduler:
    ...
    @staticmethod
    async def cleanup_user_creation_logs(retention_days: int = 90) -> Dict[str, Any]:
        """
        清理旧的用户创建日志
    ...
    @staticmethod
    async def cleanup_error_logs(retention_days: int = 30) -> Dict[str, Any]:
        """
        清理旧的错误日志
    ...
    @staticmethod
    async def cleanup_activity_logs(retention_days: int = 180) -> Dict[str, Any]:
        """
        清理旧的活动日志
    ...
    @staticmethod
    async def get_log_tables_stats() -> Dict[str, Any]:
        """
        获取日志表统计信息
    ...
    @staticmethod
    async def run_daily_maintenance() -> Dict[str, Any]:
        """
        运行每日维护任务
        ...
        # 1. 清理用户创建日志（90 天）
        results['user_creation_logs'] = await MaintenanceScheduler.cleanup_user_creation_logs(90)
        
        # 2. 清理错误日志（30 天）
        results['error_logs'] = await MaintenanceScheduler.cleanup_error_logs(30)
        
        # 3. 获取统计信息
        results['stats'] = await MaintenanceScheduler.get_log_tables_stats()
    ...
    @staticmethod
    async def run_weekly_maintenance() -> Dict[str, Any]:
        """
        运行每周维护任务
        ...
        # 1. 清理活动日志（180 天）
        results['activity_logs'] = await MaintenanceScheduler.cleanup_activity_logs(180)
        ...
        try:
            ...
            logger.error(f"❌ Failed to request VACUUM: {e}")
            results['vacuum'] = {"success": False, "error": str(e)}
