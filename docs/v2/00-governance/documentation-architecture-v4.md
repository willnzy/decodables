...
| 后端专属（DDD、API） | 仅 `decodables/docs/v2/internal/04-engineering/` | 标注 `[backend]` |
| 前端专属（组件、状态） | 仅 `decodables-fe/docs/v2/internal/04-engineering/` | 标注 `[frontend]` |
| 外部内容（manual/faq/news） | 仅 `decodables-fe/docs/v2/public/` | 前端仓库管理，发布到网站 |
...
| `07-plans/` | `internal/01-project/` 或 archive | 归档或整合 |
...
- [ ] internal 文档通用模板
- [ ] 功能规格模板 (`internal/02-product/features/*.md`)
- [ ] 模块设计模板 (`internal/04-engineering/modules/*/*.md`)
- [ ] ADR 模板 (`internal/04-engineering/architecture/decisions/*.md`)
- [ ] Manual 用户教程模板
- [ ] FAQ 问答模板（含 AI 标签）
- [ ] News 文章模板（含 SEO + 社媒文案）
- [ ] AI 知识库条目模板

### 8.2 AI 知识库详细设计 🔴 TODO

- [ ] 意图分类体系（Intent Taxonomy）
- [ ] 回复格式规范
- [ ] 与 manual/faq 的同步机制
- [ ] 转人工的触发条件
