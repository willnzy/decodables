...
| 文档 | 大小 | 用途 | 前端关注点 | 状态 |
|------|------|------|----------|------|
| [message-logging-standard.md](message-logging-standard.md) | ~15KB | ⭐ 消息与日志处理规范 (Toast/Log/Sentry 分层) | ✅ useMessage Hook、错误码映射、Logger 工具 | 🟡 规划中 |

**内容覆盖**:
- **message-logging-standard.md**:
  - 三层消息架构: 用户层 (Toast) → 开发层 (Log) → 监控层 (Sentry)
  - 前端 useMessage Hook 实现规范
  - 后端结构化日志规范 (extra 字段)
  - 错误码定义与映射表 (AUTH_xxx/RES_xxx/PAY_xxx/VAL_xxx)
  - 前后端错误响应协作规范

**使用场景**:
- 🔔 实现用户消息提示 → useMessage Hook
- 📝 开发环境调试日志 → Logger 工具类
- 🚨 错误处理与上报 → Sentry 集成
- 🔄 API 错误响应处理 → 错误码映射
...
| 文档 | 大小 | 用途 | 前端关注点 | 状态 |
|------|------|------|----------|------|
| [admin-api-review.md](admin-api-review.md) | ~150KB | ⭐ Admin API 完整参考 (178 个端点详细文档) | ✅ 管理后台开发 | 🟢 完整 |
| [user-api-review.md](user-api-review.md) | ~85KB | ⭐ User API 完整参考 (123 个端点详细文档) | ✅ 前端业务逻辑开发 | 🟢 完整 |

**内容覆盖**:
- **admin-api-review.md**:
  - 178 个 Admin API 端点 (v3.40)
  - 22 个模块分类: AI Insights、AI Models、Users、Stats、Campaigns、Config、Events、Experiments、Feature Flags、Static Pages、User Creation Monitoring 等
...
**Total Documents**: 15
**Status**: 🟢 完整文档库 - 包含业务规范、数据契约、用户体验设计、数据分析、消息日志规范、项目规划
