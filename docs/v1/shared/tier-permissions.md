...
> 所有 AI 服务的消耗通过 `system_configs` 配置，支持原价/现价双定价模式。

| 功能 | 配置 Key | 说明 |
|:-----|:---------|:-----|
| AI 生图 | `credits.cost.image_generation` | 生成单张图片的积分消耗 |
| AI 生 Page | `credits.cost.page_generation` | AI 生成整页内容的积分消耗 |
| OCR/Smart Scan | `credits.cost.ocr` | OCR 识别的积分消耗 |

**定价模式**:
- 每个 AI 服务支持 `原价` 和 `现价` 两个配置
- 代码逻辑不依赖具体数值，全部从配置读取
- 未来新增 AI 服务时，只需添加配置项即可
...
| ⁵ | **试用期天数**: 通过 `system_configs.trial.default_days` 配置，Admin 可调整 |
| ⁶ | **AI 消耗**: 所有 AI 服务的积分消耗均通过 `system_configs.credits.cost.*` 配置，支持原价/现价双定价 |
