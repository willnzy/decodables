...
本文档当前记录了 **101 个 Admin API 端点**，但实际代码中存在 **143 个端点**。
...
| 模块 | 已记录 | 实际代码 | 缺失 | 优先级 |
|------|--------|----------|------|--------|
| AI Insights | 0 | 5 | +5 | P0 (全新模块) |
| Experiments | 6 | 14 | +8 | P0 (AI 分析功能) |
| Feature Flags | 5 | 9 | +4 | P1 (审计和测试) |
| AI Models | 4 | 8 | +4 | P1 (Canary 配置) |
| Moderation | 6 | 10 | +4 | P1 (举报系统) |
| System | 9 | 12 | +3 | P2 (缓存管理) |
| Config | 6 | 8 | +2 | P2 (Rate Limits) |
| Notifications | 4 | 5 | +1 | P2 |
| Events | 5 | 5 | 0 | ✅ 完整 |
| Others | - | - | +3 | P2 |

### 详细补充文档

完整的缺失端点文档（包含请求/响应示例、验证规则、限流配置等）请参考：

📄 **[api-reference-supplement.md](api-reference-supplement.md)**

该文档包含：
- ✅ 42 个缺失端点的完整文档
- ✅ 每个端点的请求参数、请求体示例、响应示例
- ✅ 验证规则、限流配置、错误码说明
- ✅ 基于实际代码生成（v3.30+ DDD Migration）

### 计划更新

建议在下一版本（v3.28）中将补充文档合并到主文档的对应章节。

### 过时端点

以下模块的部分端点可能已废弃或合并：
- Subscriptions: 文档记录 6 个，代码实际 3 个
- Tasks: 文档记录 5 个，代码实际 4 个
- Webhooks: 文档记录 5 个，代码实际 2 个
...
*最后更新: 2026-01-11*
*更新内容:
- v3.27: 识别并记录了 42 个缺失的 Admin API 端点，详细文档见补充文件（实际端点: 143, 已记录: 101）
