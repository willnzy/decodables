...
├── exceptions.py          # ⚠️  向后兼容层 (将逐步废弃)
├── worker.py              # RQ Worker 入口
├── scheduler.py           # 定时任务调度器
├── timezone_utils.py      # 时区处理工具
├── Procfile               # 部署配置
│
├── core/                  # ✨ 核心框架层 (业务无关，100% 可复用)
│   ├── auth/              # 认证框架
│   │   ├── interface.py   # IAuthProvider 接口
│   │   ├── context.py     # AuthContext 上下文
│   │   └── jwt_utils.py   # JWT 工具
│   ├── cache/             # 缓存框架
│   │   ├── interface.py   # ICacheProvider 接口
...
│   │   └── retry.py       # 重试装饰器
│   ├── exceptions/        # 异常体系
│   │   ├── base.py        # AppException, ErrorCode, ErrorResponse
│   │   ├── auth.py        # 认证异常
│   │   ├── general.py     # 通用异常
│   │   ├── resource.py    # 资源异常
│   │   └── validation.py  # 验证异常
...
│       └── pagination.py  # 分页工具
│
├── shared/                # ✨ 共享服务层 (跨 domain 共享)
│   ├── ai/                # AI 服务 (被 creation domain 使用)
│   │   ├── unified_service.py
│   │   ├── model_config.py
│   │   ├── canary_service.py
│   │   ├── usage_tracking.py
│   │   └── adapters/
│   ├── payment/           # 支付服务 (被 billing domain 使用)
│   │   └── stripe_client.py
│   └── storage/           # 存储服务 (被 creation/marketplace domain 使用)
│       └── s3_client.py
│
├── domains/               # ✨ 领域层 (DDD 核心)
│   ├── billing/           # 💰 计费领域
...
│   │   ├── value_objects.py        # Credits, CreditBucket, TransactionType
│   │   ├── service.py              # BillingService (领域服务)
│   │   ├── repository.py           # ICreditRepository (接口)
│   │   └── exceptions.py           # InsufficientCreditsException...
│   ├── identity/          # 👤 身份领域
│   │   ├── aggregates/
│   │   │   └── user_profile.py     # UserProfile 聚合根
│   │   ├── value_objects.py        # UserTier, OnboardingStep
...
│   │   ├── value_objects.py        # ProjectStatus, AssetType
│   │   ├── service.py              # CreationService
│   │   ├── repository.py           # IProjectRepository
│   │   └── exceptions.py
│   ├── marketplace/       # 🛒 市场领域
│   │   ├── aggregates/
│   │   │   └── listing.py          # Listing 聚合根
│   │   ├── value_objects.py        # ListingStatus, PriceRange
│   │   ├── service.py              # MarketplaceService
│   │   ├── repository.py           # IListingRepository
│   │   └── exceptions.py
│   └── platform/          # ⚙️  平台领域
│       ├── aggregates/
│       │   ├── feature_flag.py     # FeatureFlag 聚合根
│       │   └── experiment.py       # Experiment 聚合根
│       ├── value_objects.py        # FlagStatus, ExperimentVariant
│       ├── service.py              # PlatformService
│       ├── repository.py           # IFeatureFlagRepository
│       └── exceptions.py
│
├── application/           # ✨ 应用层 (用例编排)
│   ├── commands/          # 写操作 (Command)
│   │   ├── billing.py     # DeductCreditsCommand, AddCreditsCommand
│   │   ├── identity.py    # CreateUserCommand, UpdateTierCommand
│   │   ├── creation.py    # CreateProjectCommand, UpdateProjectCommand
│   │   ├── marketplace.py # CreateListingCommand, PublishListingCommand
│   │   └── platform.py    # CreateFeatureFlagCommand, CreateExperimentCommand
│   └── queries/           # 读操作 (Query)
│       ├── billing.py     # GetUserCreditsQuery, GetTransactionHistoryQuery
│       ├── identity.py    # GetUserQuery, CheckFeatureAccessQuery
│       ├── creation.py    # GetProjectQuery, ListProjectsQuery
│       ├── marketplace.py # GetListingQuery, SearchListingsQuery
│       └── platform.py    # EvaluateFeatureFlagQuery, GetExperimentVariantQuery
│
├── infrastructure/        # ✨ 基础设施层 (技术实现)
│   ├── repositories/      # Repository 实现
│   │   ├── credit_repository.py    # SupabaseCreditRepository
│   │   ├── user_repository.py      # SupabaseUserRepository
│   │   ├── project_repository.py   # SupabaseProjectRepository
│   │   ├── listing_repository.py   # SupabaseListingRepository
│   │   ├── feature_flag_repository.py
│   │   └── experiment_repository.py
│   ├── external_services/ # 第三方服务集成
│   │   ├── clerk_client.py
│   │   └── fal_client.py
│   └── messaging/         # 消息队列
│       └── redis_queue.py
│
├── api/                   # ✨ API 层 (HTTP 接口)
│   ├── billing_api.py     # /credits/* endpoints
│   ├── user_api.py        # /users/* endpoints
│   ├── projects_api.py    # /projects/* endpoints
│   ├── marketplace_api.py # /marketplace/* endpoints
│   ├── platform_api.py    # /feature-flags/*, /experiments/*
│   ├── generation_api.py  # /generation/* endpoints
│   ├── webhooks_clerk.py  # Clerk webhooks
│   ├── webhooks_stripe.py # Stripe webhooks
│   └── admin/             # 管理后台 API
│       └── ...
│
├── routers/               # ⚠️  旧版 API (v1/v2)，逐步废弃中
│   └── ...                # 向后兼容，新功能请使用 api/*
│
├── services/              # ⚠️  旧版服务层，逐步废弃中
│   ├── db/                # → 已迁移到 infrastructure/repositories/
│   ├── ai/                # → 已迁移到 shared/ai/
│   ├── payment/           # → 已迁移到 shared/payment/
│   └── ...                # 其他服务仍在使用中，需逐步迁移
│
├── schemas/               # Pydantic 数据模型 (DTO)
│   ├── base.py            # 通用响应模型
│   ├── users.py           # 用户相关 DTO
│   ├── projects.py        # 项目相关 DTO
│   ├── marketplace.py     # 市场相关 DTO
│   └── ...
│
...
│
├── tests/                 # 测试
│   ├── test_app_startup.py        # ✅ 启动烟雾测试
│   ├── domains/                   # ✅ Domain 层单元测试
│   │   ├── test_billing_domain.py
│   │   ├── test_identity_domain.py
│   │   ├── test_creation_domain.py
│   │   ├── test_marketplace_domain.py
│   │   └── test_platform_domain.py
│   ├── application/               # Application 层测试
│   │   ├── test_billing_handlers.py
│   │   ├── test_platform_handlers.py
│   │   └── ...
│   ├── integration/               # ✅ 集成测试
│   │   ├── test_billing_flow.py
│   │   ├── test_credit_purchase_flow.py
...
└── docs/                  # 文档
    ├── 后台业务逻辑说明.md
    ├── DDD-Migration-Guide.md      # ✨ DDD 迁移指南 (新增)
