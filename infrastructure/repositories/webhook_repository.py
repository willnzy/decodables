...
class SupabaseWebhookRepository:
    ...
    @retry_on_network_error()
    async def create_clerk_webhook_event(
        self,
        event_id: str,
        event_type: str,
        payload: Dict[str, Any],
        ...
        """
        Create new Clerk webhook event record.

        Args:
            event_id: Clerk event ID (unique)
        ...
        try:
            ...
            if "duplicate key" in str(e).lower():
                logger.info(f"Clerk webhook event {event_id} already exists (idempotent)")
                return None
            logger.error(f"Failed to create Clerk webhook event: {e}")
            raise

    @retry_on_network_error()
    async def update_clerk_webhook_status(
        self,
        event_id: str,
        processed: bool,
        error_message: Optional[str] = None,
        ...
        """
        Update Clerk webhook event processing status.

        Args:
            event_id: Clerk event ID
            processed: Whether processing succeeded
            error_message: Error message if failed
            increment_retry: Whether to increment retry count
        ...
        try:
            ...
            logger.error(f"Failed to update Clerk webhook status: {e}")
            return False

    @retry_on_network_error()
    async def get_failed_clerk_webhooks(
        self,
        max_retry_count: int = 5,
        hours_since_created: int = 72,
        limit: int = 50,
        ...
        """
        Get failed Clerk webhooks eligible for retry.
        ...
        try:
            ...
            logger.error(f"Failed to get failed Clerk webhooks: {e}")
            return []
