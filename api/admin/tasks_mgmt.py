...
@router.get("/status")
async def get_tasks_status(admin: dict = Depends(require_admin)):
...
@router.get("/health")
async def get_tasks_health(admin: dict = Depends(require_admin)):
...
async def run_task_manually(task_name: str, admin: dict = Depends(require_admin)):
